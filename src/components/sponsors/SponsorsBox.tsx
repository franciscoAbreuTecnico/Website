...
export default function MySponsorsBox({ type }: MySponsorsBoxProps) {
  ...
  return (
    <section
      className={` p-5 rounded-[10px] shadow-[10px_-10px_20px_rgba(0,0,0,0.2)] text-center lg:my-[10vh] my-[5vh] mx-[10vw] ${sponsorTypeStyles[type]}`}
    >
      <h2 className=" font-bold uppercase text-white tracking-[0.5vw] inline-block pb-[10px] lg:text-3xl text-2xl">
        {sectionTitle}
      </h2>
      <div className="flex flex-wrap justify-center gap-5 mb-5">
        {sponsors.map((sponsor, index) => (
          <a
            className={`${sponsorCardSize} flex items-center justify-center bg-white rounded-md shadow-md transition-colors duration-300 hover:bg-[#b4b4b4]`}
            key={index}
            href={sponsor.link}
            target="_blank"
